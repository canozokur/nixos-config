...
let
  ...
  vmName = "win11";
  gpuVideo = config.services.virt-host.gpuPassthrough.video;
  gpuAudio = config.services.virt-host.gpuPassthrough.audio;
in
{
  options.services.virt-host.gpuPassthrough = {
    video = lib.mkOption {
      type = lib.types.str;
      default = "";
